...
def plot_geo_heatmap(query, figfolder):
    ...
    plt.text(cax_position[0] + cax_position[2] / 2 + 0.01, cax_position[1] + cax_position[3] + 0.01,
             cb_title, fontsize=cb_title_fontsize, ha='center', va='bottom', fontname='Times New Roman', weight='bold',
...
def plot_OD_map(begin, end, figfolder):
    ...
    colors = [(0, 1, 0), (1, 1, 0), (1, 0, 0)]  # 绿-黄-红的渐变色
    cmap = LinearSegmentedColormap.from_list('custom_cmap', colors, N=100)
    volume_normalized = (gdf1['od_pair_count'] - gdf1['od_pair_count'].min()) / (
            gdf1['od_pair_count'].max() - gdf1['od_pair_count'].min())
